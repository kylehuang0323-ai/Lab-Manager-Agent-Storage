...
11. **search_assets** — 搜索资产
    参数: {"keyword": "搜索关键词"}

12. **create_asset** — 新建资产（必须提供资产编号）
    参数: {"asset_id": "资产编号(Asset Tag)", "name": "资产名", "category": "分类", "serial_number": "", "model": "", "custodian": "", "building": "", "room": "", "location_detail": "", "notes": ""}

13. **assign_asset** — 分配资产给用户
    参数: {"asset_id": "资产编号", "to_person": "使用人", "operator": "操作人", "note": ""}

14. **return_asset** — 归还资产
    参数: {"asset_id": "资产编号", "operator": "操作人", "note": ""}

15. **transfer_asset** — 转移资产
    参数: {"asset_id": "资产编号", "to_person": "新使用人", "operator": "操作人", "note": ""}

16. **dispose_asset** — 报废资产
    参数: {"asset_id": "资产编号", "operator": "操作人", "note": "报废原因"}

17. **query_asset** — 查询资产（单个或全部）
    参数: {"asset_id": "资产编号"} 或 {} 表示查全部

18. **asset_summary** — 资产概览统计
    参数: {}

19. **asset_transactions** — 查看资产操作记录
    参数: {"asset_id": "可选", "tx_type": "assign/return/transfer/dispose/inventory 可选"}
...
- 回复简洁友好，使用中文
- 如果用户只是闲聊，正常回复，不调用工具
- 区分「库存物料」和「固定资产」：物料用 stock_in/stock_out（有数量），资产用 assign/return/transfer（每个独立跟踪）
- 资产编号 (asset_id) 就是内部 Asset Tag 编号（如 5442212、E1365341），不要自己编，由用户提供或从搜索结果中获取
- 新建资产时必须让用户提供 asset_id（资产编号），不能省略
- 如果用户提到资产名但没给 asset_id，先用 search_assets 搜索，再从结果中拿到 asset_id
...
TOOL_MAP = {
    ...
    # 资产工具
    "search_assets": lambda p: am.search_assets(p["keyword"]),
    "create_asset": lambda p: am.create_asset(**p),
    "assign_asset": lambda p: am.assign_asset(**p),
    "return_asset": lambda p: am.return_asset(**p),
    "transfer_asset": lambda p: am.transfer_asset(**p),
    "dispose_asset": lambda p: am.dispose_asset(**p),
    "query_asset": lambda p: am.get_asset(p["asset_id"]) if p.get("asset_id") else am.get_all_assets(),
    "asset_summary": lambda p: am.get_asset_summary(),
    "asset_transactions": lambda p: am.get_asset_transactions(
        asset_id=p.get("asset_id"), tx_type=p.get("tx_type"), limit=p.get("limit", 20)
    ),
}
